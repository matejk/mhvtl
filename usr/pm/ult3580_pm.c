...
void init_ult3580_tda(struct lu_phy_attr *lu) {
	ssc_pm.name								 = pm_name_lto10;
	ssc_pm.lu								 = lu;
	ssc_pm.native_drive_density				 = &density_lto10;
	ssc_pm.update_encryption_mode			 = update_ult_encryption_mode;
	ssc_pm.encryption_capabilities			 = encr_capabilities_ult;
	ssc_pm.kad_validation					 = td4_kad_validation;
	ssc_pm.clear_WORM						 = clear_ult_WORM;
	...
	/* LTO-10 drops backwards compatibility entirely: an Ultrium 10 drive
	 * handles only the 30TB and 40TB Premium Ultrium 10 cartridges.
	 * IBM LTO SCSI Reference GA32-0928-08, "Ultrium 10 Tape Drive".
	 */
	add_density_support(&lu->den_list, &density_lto10, 1);
	add_density_support(&lu->den_list, &density_lto10p, 1);

	add_drive_media_list(lu, LOAD_RW, "LTO10");
	add_drive_media_list(lu, LOAD_RO, "LTO10 Clean");
	add_drive_media_list(lu, LOAD_RW, "LTO10 WORM");
	add_drive_media_list(lu, LOAD_RW, "LTO10 ENCR");
	add_drive_media_list(lu, LOAD_RW, "LTO10P");
}
